...
async def print(client: Client, message: Message):
    global running
    ...
    try:
        ...
        admins_links = await app.get_chat_admins_with_invite_links(chat_id=message.chat.id)
        for admins_link in admins_links:
            count_running += 1
            await print(str(admins_link) + "\n")
        await print(f'共有 {count_running} 邀请链接')
