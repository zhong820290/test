...
@app.on_message(filters.command('kaf'))
    ...
    while True:
        try:
            ...
            async for member in members:
                ...
                if member.user.language_code is None:
                    continue
                if member.user.language_code != 'zh-hans':
                    await app.ban_chat_member(chat_id=group_id, user_id=member.user.id,
                                              until_date=datetime.now() + timedelta(seconds=35))
                    count += 1
                    print(f'已处理第 {count} 个')
